...
public class YandexSpellerSOAP {
    ...
    public static class SOAPBuilder {
        ...
        public YandexSpellerSOAP.SOAPBuilder language(Language language) {
            soapReq.params.put(PARAM_LANG,   language.getLangCode());
            return this;
        }

        public Response callSOAP() {
            String soapBody="<soapenv:Envelope xmlns:soapenv=" +
                    ...
                    +  " options=" + QUOTES +
                    (soapReq.params.getOrDefault(PARAM_OPTIONS, "0"))+ QUOTES
                    + " format=\"\">\n" +
                    "         <spel:text>"+ (soapReq.params.getOrDefault(
                            PARAM_TEXT,
                            SimpleWord.BROTHER.getWrongVersion()))
                    + "</spel:text>\n" +
                    "      </spel:"+ soapReq.action.getReqName() + ">\n" +
                    "   </soapenv:Body>\n" +
                    "</soapenv:Envelope>";
            ...
            return RestAssured.with()
                    ...
                    .header("SOAPAction", "http://speller.yandex.net/services/spellservice/"
                            + soapReq.action.getMethod())
                    .body(soapBody)
                    .log().all().with()
                    .post().prettyPeek();
        }
